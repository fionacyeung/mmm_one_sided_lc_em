...
#' summary(Elec.mm)
#' }
gmnl <- function (formula, data, subset, weights, na.action, model = c("mnl", 
                                                                       "mixl", "smnl", "gmnl", "lc", "mm"), start = NULL, ranp = NULL, 
                  ...
                  print.init = FALSE, gradient = TRUE, typeR = TRUE, ...) 
{
    ...
    if (model != "mnl") {
      ...
      theta <- switch(model, smnl = c(mean, init.tau, start.het), 
                      mixl = c(mean, start.phi, start.stds), gmnl = c(mean, 
                                                                      ...
                                                                      init.gamma), lc = c(lc.mean, start.het), mm = c(lc.mean, 
                                                                                                                      ls.mean, start.het))
      names(theta) <- allnames
    }
  }
  ...
  if (model != "mnl") {
    opt[[1]] <- switch(model, smnl = as.name("ll.smlogit"), 
                       ...
                       lc = as.name("ll.mlogitlc"), mm = as.name("ll.mnlogit"))
    if (has.rand) {
      if (!correlation) {
        if (model != "mm") {
